...
// Save the images from a set of Covisible KeyFrames
void SaveKeyFrameCovisibles(ORB_SLAM::KeyFrame* kf, const char* savename) {
    ...
    for (size_t i=0; i < closestMatches.size(); i++) {
        cv::Mat im = closestMatches[i]->GetImage();
        name = "KeyFrame" + std::to_string(i);
        WriteImage(name.c_str(), im);
    }
}


// Print the min and max pixel depths found by Stereo Search and 
// compare to the median scene depth computed by the KeyFrame
void TestMinMaxDepths (std::vector<ORB_SLAM::KeyFrame*> kfs) {
  float max_depth;
  float min_depth;
  float median_depth;
  for (size_t i=0; i < kfs.size(); i++) {
      ORB_SLAM::KeyFrame *kf = kfs[i];
      StereoSearchConstraints(kf, &min_depth, &max_depth);
      // standard for comparison
      median_depth = kf->ComputeSceneMedianDepth();
      cout << "KeyFrame " << i << ": \n";
      cout << "min depth:" << min_depth  << "\n";
      cout << "max depth:" << max_depth << "\n";
      cout << "median depth:" << median_depth << "\n";
      cout << "\n";
  }
}


// Check the computed gradient magnitude and orientation for a KeyFrame image
// and saves the result to two image files.
void TestGradientMagAndOri (const ORB_SLAM::KeyFrame* kf, const char* magsavename, const char* orisavename) {
  ...
  WriteImage(magsavename, gradmag);
  WriteImage(orisavename, gradth);
}


// Check how many high gradient areas are left after filtering by LambdaG
// and save the result to an image.
void TestLambdaG (const cv::Mat& image, const cv::Mat& imageGradMag, int _lambdaG, const char* savename) {
  cv::Mat highGradsAbs = cv::Mat::zeros(image.rows, image.cols, imageGradMagAbs.type());
  cv::Mat highGradsAbsScaled(iamge.rows, image.cols, CV_8U);
  float pixelGradMag;
  for(int x = 0; x < image.rows; x++){
    for(int y = 0; y < image.cols; y++){
        pixelGradMag = imageGradMagAbs.at<float>(x,y);
        cout << "Pixel gradient magnitude: " << pixelGradMag << "  lambdaG: " << lambdaG << "\n";
        if(pixelGradMag < _lambdaG)
            highGradsAbs.at<float>(x,y) = 0;
        else
            highGradsAbs.at<float>(x,y) = pixelGradMag;
    }
  }
  ScaleMatFloat2UChar(highGradsAbs, &highGradsAbsScaled);
  
  std::string savename2(savename);
  savename2 += "-lambdaG_";
  savename2 += std::to_string(lambdaG);
  WriteImage(savename2, highGradsAbsScaled);
}


// utility method extracted from First Loop: returns the set of depth hypotheses for a given pixel from a given KeyFrame 
void GetDepthHypothesesForPixel(int px, int py, ORB_SLAM::KeyFrame* kf, std::vector<depthHo>& depth_results) {
  depth_results.clear();
  ...
  for(size_t i=0; i<closestMatches.size(); i++){
    ...
    depthHo dh;
    unsigned char pixel = image.at<uchar>(x,y);
    EpipolarSearch(kf, kf2, x, y, pixel, gradx, grady, gradmag, min_depth, max_depth, &dh);
    if (dh.supported)
        depth_results.push_back(dh);
  }
}


// Check the pixel depths, and the number of supported pixels, after running Epipolar Search on a list of KeyFrames.
// Save the depth results ('0' indicates 'no support') to an image.
void TestSupportedAfterEpipolarSearch (const cv::Mat& image, const cv::Mat& gradmag, float min_depth, float max_depth, const std::vector<ORB_SLAM::KeyFrame*>& closestMatches, const char* savename) {
    cv::Mat supported = cv::Mat::zeros(image.rows, image.cols, CV_32F);
    cv::Mat supportedScaled(image.rows, image.cols, CV_8U);
    int depth_err_count = 0;
    for(int x = 0; x < image.rows; x++){
        for(int y = 0; y < image.cols; y++){
            if(gradmag.at<float>(x,y) < lambdaG)
                continue;
            for(size_t i=0; i<closestMatches.size(); i++){
                ORB_SLAM::KeyFrame* kf2 = closestMatches[i];
                depthHo dh;
                unsigned char pixel = image.at<uchar>(x,y); //maybe it should be cv::Mat\
                // Pixel Depth from Epipolar Search
                EpipolarSearch(kf, kf2, x, y, pixel, gradx, grady, gradmag, min_depth, max_depth, &dh);
                cout << "Depth: " << dh.depth << "\n";
                // Check for Depth < 0
                if (dh.depth < 0) {
                ...
                if (dh.supported)
                    supported.at<float>(x,y) = dh.depth;
                else
                    supported.at<float>(x,y) = 0;
            }
        }
    }
    cout << "Found " << depth_err_count << " depths < 0 with " << closestMatches.size() << " KeyFrames and " << image.rows * image.cols << " pixels\n";
    ScaleMatFloat2UChar(supported, &supportedScaled);
    WriteImage(savename, supportedScaled);
}


// Test the Inverse Depth Hypothesis Value for pixels in a KeyFrame image.
// Save the inverse depth results to an image.
void TestInverseDepthHypothesisFusion (ORB_SLAM::KeyFrame* kf, const char* savename) {
    cv::Mat im = kf->GetImage();
    
    cv::Mat invDepths(im.rows, im.cols, CV_32F);
    cv::Mat invDepthsScaled(im.rows, im.cols, CV_8U);
    
    std::vector<depthHo> depth_results;
    for (int x = 0; x < im.rows; x++) {
        for (int y = 0; y < im.cols; y++) {
            depth_results.clear();
            GetDepthHypothesesForPixel(x, y, kf, depth_results);
            depthHo dh;
            InverseDepthHypothesisFusion(depth_ho, &dh);
            invDepths.at<float>(x,y) = dh.depth;
        }
    }
    ScaleMatFloat2UChar(invDepths, &invDepthsScaled);
    WriteImage(savename, invDepthsScaled);
}
...
void ProbabilityMapping::EpipolarSearch(ORB_SLAM::KeyFrame* kf1, ORB_SLAM::KeyFrame *kf2, int x, int y, unsigned char pixel, cv::Mat gradx, cv::Mat grady, cv::Mat grad, 
...
//FIXME ASAP
    ...
    unsigned char photometric_err = pixel - image.at<uchar>(uj,vj); //FIXME properly calculate photometric error
...
void ProbabilityMapping::InterKeyFrameDepthChecking(const cv::Mat& im, ORB_SLAM::KeyFrame* currentKf, std::vector<std::vector<depthHo> >& h) {
    ...
    std::vector<cv::Mat> sdpmMapPoints;
    for (int px = 0; px < im.rows; px++) {
        for (int py = 0; py < im.cols; py++) {
            ...
            // copy point into MapPoint vector
            cv::Mat pos(3, 1, CV_32F); // x, y, z
            pos.at<float>(0) = px;
            pos.at<float>(1) = py;
            pos.at<float>(2) = h[px][py].depth;
            sdpmMapPoints.push_back(pos);
        } // for py = 0...im.cols-1
    } // for px = 0...im.rows-1

    pub->SetPoints(sdpmMapPoints);
} 
...
void ProbabilityMapping::GetGradientMagAndOri(const cv::Mat& image, cv::Mat* gradx, cv::Mat* grady, cv::Mat* mag, cv::Mat* ori, cv::Mat* really) {
  ...
  *ori = cv::Mat::zeros(image.rows, image.cols, CV_32F);
  ...
  //print gradient and theta results to file
  
  std::vector<std::string> names;
  names.push_back("manual_grad_mag.csv");
  names.push_back("cv_grad_mag.csv");
  names.push_back("cv_grad_theta.csv");
  
  std::vector<cv::Mat*> mats;
  mats.push_back(really);
  mats.push_back(mag);
  mats.push_back(ori);
  
  print_csv(names, mats);
...
void print_csv(std::vector<std::string> names, std::vector<cv::Mat*> mats) {
    std::string basepath = "/home/acceber/projects/eecs395/"; 
    for (int i=0; i<names.size(); i++) {
    std::string filename = basepath + names[i];
