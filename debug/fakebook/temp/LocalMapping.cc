...
void LocalMapping::SetSdpmMapper(ProbabilityMapping* pSdpmMapper) {
    
    mSdpmMapper = pSdpmMapper;
}
...
void LocalMapping::Run()
{
    ...
    while(ros::ok())
    {
        ...
        {            
            ...
            // Create new semi-dense map points
            CreateNewSdpmMapPoints();
...
void LocalMapping::CreateNewSdpmMapPoints() {
    vector<KeyFrame*> vpNeighKFs = mpCurrentKeyFrame->GetBestCovisibilityKeyFrames(20);
    for (size_t n = 0; n < vpNeighKFs.size(); n++) {
        ORB_SLAM::KeyFrame* pKF = vpNeighKFs[n];
